...
export const DispensingLog: React.FC<DispensingLogProps> = ({ user }) => {
  ...
  const handleVoid = async () => {
    ...
    try {
      if (!window.electron?.staff?.verifyAction || !window.electron?.dispensing?.void) {
        alert('API is not available. Please restart the application.');
        return;
      }
      const verified = await window.electron.staff.verifyAction(pin, 'void') as {
        success: boolean;
        staff?: { id: number };
      };
      if (!verified.success) {
      ...
      await window.electron.dispensing.void({
        ...
        voided_by: verified.staff?.id ?? user.id,
        reason: voidReason,
      });
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg w-full max-w-3xl max-h-[90vh] overflow-auto">
            ...
            <div className="p-6 border-t border-gray-200 flex justify-between">
              ...
              {selectedRecord.status === 'completed' && user.role !== 'staff' && (
                <button
