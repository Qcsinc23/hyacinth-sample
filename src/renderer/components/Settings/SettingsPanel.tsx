...
import { InstructionTemplatesPanel } from './InstructionTemplatesPanel';
...
export const SettingsPanel: React.FC<SettingsPanelProps> = ({ className = '' }) => {
    ...
    ...(isAdmin
      ? [
          ...
          },
          {
            ...
            label: 'Dispensing Instructions',
            icon: <FileText className="h-5 w-5" />,
            description: 'Edit pre-programmed label instructions',
          },
        ]
      : []),
    {
      ...
      if (result) {
        ...
        setStatusMessage(
          `${type} data exported successfully${
            typeof result === 'object' && result && 'filename' in result
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'instructions':
        return <InstructionTemplatesPanel />;
