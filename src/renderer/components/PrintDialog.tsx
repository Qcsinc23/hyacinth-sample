...
interface PrintDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (options: PrintDialogOptions) => void;
  patientName: string;
  chartNumber: string;
...
export const PrintDialog: React.FC<PrintDialogProps> = ({
  ...
  const handlePreview = async () => {
    setIsLoading(true);
    try {
      // Generate preview - this would create temporary PDFs and open them
      // For now, just show a message
      alert('Preview functionality will open generated PDFs in your default viewer.');
    } finally {
      setIsLoading(false);
    }
  };
