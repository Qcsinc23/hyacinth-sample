...
export function useDatabase() {
  ...
  const searchPatients = useCallback(async (query: string): Promise<Patient[]> => {
    ...
    try {
      ...
      const response = await window.electron.patient.search(query);
      const list = Array.isArray(response)
        ? response
        : (response?.data ?? []);
      return (list as any[]).map((p) => ({
        id: String(p.id),
        chartNumber: p.chart_number ?? p.chartNumber ?? '',
        firstName: p.first_name ?? p.firstName ?? '',
        lastName: p.last_name ?? p.lastName ?? '',
        dateOfBirth: p.dob ? new Date(p.dob) : (p.dateOfBirth ? new Date(p.dateOfBirth) : new Date()),
  ...
  const getDispenseRecords = useCallback(async (filters?: {
    ...
    try {
      ...
      const toRecordStatus = (s: string): 'completed' | 'corrected' | 'voided' => {
        return ['completed', 'corrected', 'voided'].includes(s)
          ? (s as 'completed' | 'corrected' | 'voided')
          : 'completed';
      };
      ...
      const records = rawList.map((r: any) => {
        ...
        return {
          ...
          status: toRecordStatus(r.status || 'completed'),
