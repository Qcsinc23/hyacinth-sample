...
/** NJ Board of Pharmacy (N.J.A.C. 13:39-7.12) compliant clinic information */
...
export interface LabelData {
  ...
  dosageForm?: string; // e.g., "Tablet", "Injection", "Capsule" - NJ requires form
  quantity: number;
  unit: string;
  daySupply: number;
  directions: string;
  ...
  lotNumber?: string;
  expirationDate?: string; // Use By date - NJ required for non-manufacturer packaging
  qrCodeData?: string;
  /** NJ: "Generic for [Brand]" when dispensing generic */
  genericFor?: string;
  // Enhanced fields for context-specific printing
  instructionContext: ReasonContext;
  fullWarnings: string[]; // All warnings - NJ auxiliary labeling
...
async function drawAveryLabel(
  ...
  // Medication name, strength, and form (NJ compliant)
  const averyMedDisplay = data.dosageForm
    ? `${data.medicationName} ${data.medicationStrength} ${data.dosageForm}`
    : `${data.medicationName} ${data.medicationStrength}`;
  page.drawText(averyMedDisplay.toUpperCase(), {
    x: x + margin,
    y: currentY,
    font: boldFont,
    size: 8,
...
async function drawLabelContent(
  ...
  // NJ 13:39-7.12: Pharmacy/Clinic name, address, and telephone
  page.drawText(CLINIC_INFO.name.toUpperCase(), {
    x: margin,
    y,
    font: boldFont,
    size: 12,
  ...
  y -= 16;
  page.drawText(CLINIC_INFO.address, {
    x: margin,
    y,
    font,
    size: 9,
  ...
  y -= 14;
  page.drawText(CLINIC_INFO.phone, {
    x: margin,
    y,
    font,
    size: 10,
  ...
  // NJ: Patient name (bold, larger - enhanced readability)
  page.drawText(data.patientName.toUpperCase(), {
    x: margin,
    y,
    font: boldFont,
  ...
  // NJ: Medication name, strength, and form (N.J.A.C. 13:39-7.12)
  ...
  y -= 15;
  const medDisplay = data.dosageForm
    ? `${data.medicationName} ${data.medicationStrength} ${data.dosageForm}`
    : `${data.medicationName} ${data.medicationStrength}`;
  page.drawText(medDisplay.toUpperCase(), {
    x: margin,
    y,
    font: boldFont,
    size: 11,
  ...
  y -= 14;
  if (data.genericFor) {
    page.drawText(`Generic for ${data.genericFor}`, {
      x: margin,
      y,
      font,
      size: 9,
  ...
  // NJ: Use By / Expiration date (required for non-manufacturer packaging)
  if (data.lotNumber || data.expirationDate) {
    ...
    if (data.expirationDate) {
      const useByText = `Use By: ${data.expirationDate}`;
      page.drawText(useByText, {
        x: page.getWidth() - margin - useByText.length * 5.5,
        y,
        font: boldFont,
        size: 9,
      });
