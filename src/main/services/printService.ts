...
export function registerPrintIpcHandlers(): void {
  const success = <T>(data: T): IpcResponse<T> => ({ success: true, data });
  const failure = (error: unknown): IpcResponse<never> => ({
    success: false,
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        ...
        return success<PrintSubmissionResult>({
          jobId: job.id,
          type: job.type,
          status: job.status,
  ...
  ipcMain.handle(
    ...
    (_event: IpcMainInvokeEvent, jobId: string) => {
      return success({ cancelled: cancelJob(jobId) });
    },
  );
