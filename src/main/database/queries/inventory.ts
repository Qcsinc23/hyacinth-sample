...
export function getInventoryById(id: number): Inventory | null {
  ...
  return db
    ...
    .get(id) as Inventory | null;
}

/**
 * Get inventory with transactions
 */
export function getInventoryWithTransactions(
  id: number,
): InventoryWithTransactions | null {
  const db = getDatabase();
 ...
 */
export function getInventoryByMedication(
...
export function checkAndCreateAlerts(inventoryId: number): void {
  ...
  if (inventory.expiration_date < today) {
    ...
    db.prepare(
      `UPDATE inventory SET status = 'expired', updated_at = ? WHERE id = ?`,
    ).run(now, inventoryId);
    ...
    if (!existing) {
      const daysUntilExpiry = Math.ceil(
        (new Date(inventory.expiration_date).getTime() -
 ...
 */
export function quarantineInventory(
  inventoryId: number,
  reason: string,
  _staffId: number,
): Inventory {
...
export function validateLotForDispensing(
  ...
  if (daysUntilExpiration < 0) {
    errors.push(
      `This lot has expired on ${lot.expiration_date}. Cannot dispense.`,
    );
  } else if (daysUntilExpiration < 30) {
    warnings.push(
      `Lot expires in ${daysUntilExpiration} day${daysUntilExpiration !== 1 ? 's' : ''}. Consider using newer stock.`,
    );
  ...
  if (lot.quantity_on_hand < quantity) {
    errors.push(
      `Insufficient quantity. Available: ${lot.quantity_on_hand} ${lot.unit}, Requested: ${quantity} ${lot.unit}`,
    );
  }
  ...
  if (lot.status === 'depleted') {
    errors.push(
      'This lot has been depleted and is not available for dispensing.',
    );
 ...
 */
export function getAvailableLotsForDispensing(
  medicationName: string,
): ExtendedLotInfo[] {
 ...
 */
export function getLotsExpiringWithin(
  days: number,
  includeExpired = false,
): ExtendedLotInfo[] {
  const db = getDatabase();
...
export function getLotByNumber(lotNumber: string): Inventory | null {
  const db = getDatabase();
  return db
    .prepare('SELECT * FROM inventory WHERE lot_number = ?')
    .get(lotNumber) as Inventory | null;
}
 ...
 */
export function validateLotsForDispensing(
  items: Array<{ lotId: number; quantity: number }>,
): Array<LotValidationResult & { lotId: number }> {
