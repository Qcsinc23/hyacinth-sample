...
import { closeDatabase, getDbPath, initDatabase } from '../database/db';
...
export const restoreFromBackup = async (
  ...
  try {
    ...
    try {
      // Copy backup to current database location and re-open it immediately so
      // integrity problems fail within the restore flow instead of on next launch.
      fs.copyFileSync(sourcePath, currentDbPath);
      const restoredDb = initDatabase();
      const integrityCheck = restoredDb.pragma('integrity_check', { simple: true });
      if (integrityCheck !== 'ok') {
        throw new Error(`Restored database integrity check failed: ${integrityCheck}`);
      }
    } catch (restoreError) {
      if (fs.existsSync(currentBackupPath)) {
        log.warn('Restore failed; rolling database back to pre-restore snapshot');
        fs.copyFileSync(currentBackupPath, currentDbPath);
        try {
          initDatabase();
        } catch (rollbackError) {
          log.error(
            'Rollback after failed restore did not re-open cleanly:',
            rollbackError,
          );
        }
      }
